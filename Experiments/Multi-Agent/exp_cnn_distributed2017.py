...
def experiment(base_index=0, start_index=0, build_base=True, path_base='base-memory-cnn', path_fisher='fisher.npy'):
    ...
    if build_base:
        ...
        with open('tem_fisher_cnn', 'wb') as f:
            np.save(f, fisher_matrix)
    else:
        fisher_matrix = np.load('tem_fisher_cnn', allow_pickle=True)
    ...
    half_trained_cnn = distill_models(base_index=base_index, base_cnn=base_cnn,
                                      distillation_train_data=distillation_train_data,
    ...
    for layer in range(len(fisher_matrix)):
        fisher_matrix[layer] = (number_of_agents_training_samples / number_of_all_data) * temp.fisher_matrix[
            layer] + (number_of_train_samples / number_of_all_data) * fisher_matrix[layer]
...
def expand_models(base_cnn, number_of_train_samples, attacks, original_attacks, original_labels):
    ...
    for i in range(len(attacks)):
        ...
        tf.keras.models.save_model(expanded_model, 'expanded_cnn',
                                       save_format='tf')
        #expanded_model.save('expanded_cnn')
...
def distill_models(base_index, base_cnn, distillation_train_data, distillation_train_logits,
                   distillation_train_labels, number_of_train_samples, attacks, original_attacks, original_labels,
                   fisher_matrix,
                   initial_accuracy, save_number):
    normals = ['vectorize_friday/benign', 'Benign_Wednesday']
    ...
    broadcaster = BroadCaster_Thread(controller=async_controller, agent_clients={},
                                     data_controller=new_data_controller,
                                     original_data_controller=old_data_controller, main_attack=attacks[0],
                                     initial_accuracy=initial_accuracy,
                                     save_path='../../Results/resluts-cnn-distributed2017/exp' + str(base_index) + 'part' + str(
