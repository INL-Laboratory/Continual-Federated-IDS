...
# 	path = '/mnt/inl-bkp2/behzad/iscxIDS2017/pcap/' # 2017
	path = '/mnt/INL-Backup/bkp-Home/soltani/2017-vectorize/' # 2017
# 	path = '/mnt/INL-Backup/bkp-Home/soltani/vectorize/' # 2018
...
class DataController(object):
    def __init__(self, data_str=data_str, batch_size=20, data_list=[],mode = 'binary',reporter = Reporter(),should_report = False,report_path = 'data_report.txt',flatten = True,max_attack_flow = 1000):
        ...
        if number_of_benign != 0:
            ...
            for i, index in enumerate(indexes_of_benigns):
                ...
                size_of_each_benign = int(
                    (number_of_attack_samples - number_of_benign_samples) / (len(indexes_of_benigns) - i))
        if should_report:
    ...
    def parse_flow(self, filename):
        ...
        if self.flatten:
            ...
            #in this case, since we only want the packets, we exclude the first row which contains meta data
